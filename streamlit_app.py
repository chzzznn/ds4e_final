...
[data-testid="stSidebar"] { background-color: #13162B; border-right: 1px solid #2A2D4A; }
[data-testid="stSidebar"] * { color: #C8C8E0 !important; }

.metric-card {
    background: linear-gradient(135deg, #1A1D35 0%, #1F2240 100%);
    border: 1px solid #2E3260; border-radius: 16px; padding: 24px;
    text-align: center; transition: transform 0.2s;
}
.metric-card:hover { transform: translateY(-3px); }
.metric-value { font-family: 'Syne', sans-serif; font-size: 2.4rem; font-weight: 800; color: #7C9EFF; line-height: 1; }
.metric-label { font-size: 0.8rem; color: #8888AA; text-transform: uppercase; letter-spacing: 1.5px; margin-top: 6px; }

.hero-section {
    background: linear-gradient(135deg, #0D0F1A 0%, #1A1040 50%, #0D1A2E 100%);
    border: 1px solid #2A2D4A; border-radius: 24px; padding: 60px 40px;
...
.hero-title { font-family: 'Syne', sans-serif; font-size: 3.5rem; font-weight: 800; color: #FFFFFF; line-height: 1.1; margin-bottom: 16px; }
.hero-subtitle { font-size: 1.15rem; color: #9090B0; max-width: 600px; line-height: 1.7; }
.accent { color: #7C9EFF; }
.tag {
    display: inline-block; background: rgba(124,158,255,0.12);
    border: 1px solid rgba(124,158,255,0.3); color: #7C9EFF; border-radius: 20px;
    padding: 4px 14px; font-size: 0.75rem; font-weight: 600; letter-spacing: 1px;
    text-transform: uppercase; margin-bottom: 20px;
}
.section-title { font-family: 'Syne', sans-serif; font-size: 1.6rem; font-weight: 700; color: #E8E8F0; margin-bottom: 6px; }
.section-sub { color: #7070A0; font-size: 0.9rem; margin-bottom: 24px; }
.insight-box {
    background: linear-gradient(135deg, rgba(124,158,255,0.08), rgba(100,200,160,0.05));
    border-left: 3px solid #7C9EFF; border-radius: 0 12px 12px 0;
    padding: 16px 20px; margin: 12px 0; font-size: 0.9rem; color: #C0C0D8; line-height: 1.6;
}
.warn-box {
    background: rgba(255,180,0,0.07); border-left: 3px solid #FFB400;
    border-radius: 0 12px 12px 0; padding: 12px 16px; margin: 8px 0;
    font-size: 0.85rem; color: #C8B060; line-height: 1.6;
}
.stSelectbox > div > div { background-color: #1A1D35 !important; border-color: #2E3260 !important; color: #E8E8F0 !important; }
.stSlider > div > div > div { background-color: #7C9EFF !important; }
div[data-testid="stMetric"] { background: #13162B; border: 1px solid #22264A; border-radius: 12px; padding: 16px; }
...
def get_model_data():
    ...
    # Candidate features — we drop any with >50% missing after zero→NaN
    # to prevent median imputation from destroying signal
    candidate_features = [
        "Birth_Rate",
    ...
    df_model = df[available + [target, "Countries and areas"]].copy()
    df_model = df_model.replace(0, np.nan)
    df_model = df_model.dropna(subset=[target])

    # Drop features >50% missing — they add noise, not signal
    missing_pct = df_model[available].isna().mean()
    good_features = missing_pct[missing_pct < 0.50].index.tolist()

    df_model = df_model[good_features + [target, "Countries and areas"]]

    X_raw  = df_model[good_features].copy()
    y      = df_model[target].values
    countries = df_model["Countries and areas"].values

    # ── CORRECT: split FIRST, then fit imputer & scaler on train only ──
    ...
    X_train_imp = pd.DataFrame(imputer.fit_transform(X_train_raw), columns=good_features)
    X_test_imp  = pd.DataFrame(imputer.transform(X_test_raw),      columns=good_features)
...
if "Overview" in page:
    ...
    with col1:
        st.markdown("""<div class='metric-card'><div class='metric-value'>202</div><div class='metric-label'>Countries</div></div>""", unsafe_allow_html=True)
    with col2:
        st.markdown("""<div class='metric-card'><div class='metric-value'>27</div><div class='metric-label'>Features</div></div>""", unsafe_allow_html=True)
    with col3:
        st.markdown(f"""<div class='metric-card'><div class='metric-value'>{valid_unemp.mean():.1f}%</div><div class='metric-label'>Avg Unemployment</div></div>""", unsafe_allow_html=True)
    with col4:
        st.markdown(f"""<div class='metric-card'><div class='metric-value'>{valid_unemp.max():.1f}%</div><div class='metric-label'>Max Unemployment</div></div>""", unsafe_allow_html=True)
    ...
    with col_l:
        ...
        like enrollment rates, out-of-school rates, completion rates, and birth rate.
        </div>
        """, unsafe_allow_html=True)
    ...
    with col_r:
        ...
        for k, v in {
            ...
            "Gross Enrollment": "Primary & Tertiary (gross %)",
            "Out-of-School Rates": "Male & Female across education levels",
            "Completion Rates": "Primary & Upper Secondary (M/F)",
        }.items():
            st.markdown(f"""
    ...
    with col_b:
        ...
        fig_miss = go.Figure(go.Bar(
            ...
            orientation="h", marker=dict(color="#7C9EFF", opacity=0.8)
        ))
        fig_miss.update_layout(
            height=400, paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
            font=dict(color="#9090B0"), xaxis=dict(gridcolor="#1E2140"),
            yaxis=dict(gridcolor="#1E2140"), margin=dict(l=0, r=0, t=10, b=0)
        )
...
# ═══════════════════════════════════════════════════════════
    ...
    (X_train_s, X_test_s, y_train, y_test,
     ...
     countries, features, imputer, scaler) = get_model_data()
    ...
    with col_sel:
        ...
        primary_model = st.selectbox("Primary model for prediction",
    ...
    with col_info:
        ...
        c1.metric("Total Samples", len(y_train) + len(y_test))
        c2.metric("Train Samples", len(y_train))
        c3.metric("Test Samples",  len(y_test))

    st.markdown(f"""<div class='warn-box'>
        ⚠️ <b>Active features ({len(features)}):</b> Features with &gt;50% missing values were excluded
        to prevent imputation noise. Using: {', '.join(features)}.
    </div>""", unsafe_allow_html=True)
    ...
    for name in selected_models:
        ...
        cv_scores = cross_val_score(MODELS[name], X_train_s, y_train,
                                    cv=min(5, len(y_train)), scoring="r2")
        results[name] = {
            "R² Score":       round(r2_score(y_test, preds), 4),
            "RMSE":           round(np.sqrt(mean_squared_error(y_test, preds)), 4),
            "MAE":            round(mean_absolute_error(y_test, preds), 4),
            "CV R² (5-fold)": round(cv_scores.mean(), 4),
        }
    ...
    for metric, color in zip(["R² Score", "RMSE", "MAE"], ["#7C9EFF", "#FF6B9D", "#64C8A0"]):
        fig_compare.add_trace(go.Bar(name=metric, x=results_df["Model"], y=results_df[metric], marker_color=color))
    fig_compare.update_layout(
        ...
        xaxis=dict(gridcolor="#1E2140"), yaxis=dict(gridcolor="#1E2140"), margin=dict(t=10)
    )
    ...
    fig_avp.add_trace(go.Scatter(x=y_test, y=preds_best, mode="markers",
                                  marker=dict(color="#7C9EFF", size=8, opacity=0.7), name="Countries"))
    ...
    slider_cfg = {
        "Birth_Rate":                             ("Birth Rate", 5.0, 50.0, 18.0),
        "Gross_Primary_Education_Enrollment":     ("Primary Enrollment (%)", 50, 150, 105),
        "Gross_Tertiary_Education_Enrollment":    ("Tertiary Enrollment (%)", 0, 120, 40),
        "OOSR_Pre0Primary_Age_Male":              ("OOSR Pre-Primary Male (%)", 0, 100, 50),
        "OOSR_Pre0Primary_Age_Female":            ("OOSR Pre-Primary Female (%)", 0, 100, 50),
        "OOSR_Primary_Age_Male":                  ("OOSR Primary Male (%)", 0, 100, 20),
        "OOSR_Primary_Age_Female":                ("OOSR Primary Female (%)", 0, 100, 22),
        "OOSR_Lower_Secondary_Age_Male":          ("OOSR Lower Secondary Male (%)", 0, 100, 30),
        "OOSR_Lower_Secondary_Age_Female":        ("OOSR Lower Secondary Female (%)", 0, 100, 32),
        "OOSR_Upper_Secondary_Age_Male":          ("OOSR Upper Secondary Male (%)", 0, 100, 40),
        "OOSR_Upper_Secondary_Age_Female":        ("OOSR Upper Secondary Female (%)", 0, 100, 42),
        "Completion_Rate_Primary_Male":           ("Primary Completion Male (%)", 0, 100, 75),
        "Completion_Rate_Primary_Female":         ("Primary Completion Female (%)", 0, 100, 72),
        "Completion_Rate_Upper_Secondary_Male":   ("Upper Secondary Male (%)", 0, 100, 55),
        "Completion_Rate_Upper_Secondary_Female": ("Upper Secondary Female (%)", 0, 100, 52),
    }
    ...
    with st.form("prediction_form"):
        ...
        for i, feat in enumerate(features):
            cfg = slider_cfg.get(feat)
            if cfg:
                label, mn, mx, default = cfg
                with cols[i % 3]:
                    if isinstance(mn, float) or isinstance(default, float):
                        vals[feat] = st.slider(label, float(mn), float(mx), float(default))
                    else:
                        vals[feat] = st.slider(label, int(mn), int(mx), int(default))
...
# ═══════════════════════════════════════════════════════════
    ...
    X_test_named = pd.DataFrame(X_test_s, columns=features)
    ...
    input_data = X_test_named if model_choice != "Linear Regression" else pd.DataFrame(X_test_s, columns=features)
    shap.summary_plot(shap_values, input_data, feature_names=features, show=False, color_bar=True, plot_size=None)
    ...
    sample_data = (X_test_named.iloc[sample_idx] if model_choice != "Linear Regression"
                   else pd.Series(X_test_s[sample_idx], index=features))

    fig_wf, _ = plt.subplots(figsize=(10, 4))
    fig_wf.patch.set_facecolor("#0D0F1A")
    shap.waterfall_plot(
        shap.Explanation(values=shap_values[sample_idx], base_values=base_val,
                         data=sample_data, feature_names=features),
        show=False
    )
    plt.gcf().patch.set_facecolor("#0D0F1A")
    for ax_ in plt.gcf().get_axes():
    ...
    💡 <b>Key Insight:</b> According to SHAP, <b>{top_feat.replace('_', ' ')}</b> is the most influential
    variable for the <b>{model_choice}</b> model. Positive SHAP = pushes unemployment higher; negative = lower.
    </div>
    <div class='insight-box'>
    💡 <b>Policy Takeaway:</b> Reducing out-of-school rates at secondary level and increasing tertiary
    enrollment are the levers with the highest predicted impact on reducing unemployment globally.
    </div>
    """, unsafe_allow_html=True)
...
# ═══════════════════════════════════════════════════════════
    ...
    with col_wb2:
        ...
        <div style='background:#13162B; border:1px solid #22264A; border-radius:12px; padding:16px; margin-top:8px;'>
            ...
            Experiments always run locally. If a W&B API key is provided, results are also logged
            to your W&B dashboard. Get your key at <b>wandb.ai/settings</b>.
            </div>
        </div>""", unsafe_allow_html=True)
    ...
    if run_tuning:
        ...
        if use_wandb:
            ...
            try:
                ...
                st.success("✅ W&B login successful!")
        ...
        fig_runs = px.bar(results_df.reset_index(drop=True),
                           x=results_df.reset_index(drop=True).index, y="R²", color="Model",
                           color_discrete_sequence=["#7C9EFF", "#FF6B9D", "#64C8A0"],
                           labels={"index": "Run #"})
        fig_runs.update_layout(
            height=350, paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(13,15,26,0.8)",
            font=dict(color="#9090B0"), xaxis=dict(gridcolor="#1E2140"),
            yaxis=dict(gridcolor="#1E2140"), legend=dict(bgcolor="rgba(0,0,0,0)"), margin=dict(t=10)
        )
        ...
        if use_wandb:
            ...
            Visit <a href='https://wandb.ai' style='color:#7C9EFF;'>wandb.ai</a> to view your dashboard.
            </div>""", unsafe_allow_html=True)
    else:
        st.markdown("""
